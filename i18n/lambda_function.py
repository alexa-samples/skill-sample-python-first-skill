...
class HasBirthdayLaunchRequestHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        attr = handler_input.attributes_manager.persistent_attributes
        year = int(attr['year'])
        month = int(attr['month']) # month is a string, and we need to convert it to a month index later
        day = int(attr['day'])
        data = handler_input.attributes_manager.request_attributes["_"]
...
class BirthdayIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        # months abbreviations vary from locale to locale, if there is an error, we capitalize the month and try again
        try:
...
class LocalizationInterceptor(AbstractRequestInterceptor):
    ...
    def process(self, handler_input):
        skill_locale = handler_input.request_envelope.request.locale
        logger.info("Locale is {}".format(skill_locale[:2]))
